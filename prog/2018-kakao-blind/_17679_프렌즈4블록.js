 ...
 * 이때, 지워지는 블록은 총 몇 개인지 판단하는 프로그램을 제작하자.
 *
 * @param {*} m 판의 높이 (2~30)
 * @param {*} n 판의 폭
 ...
 * ### 리뷰
 * - 풀이 방식은 다음과 같다.
 *   먼저, 맵을 순회하여 제거할 수 있는 블록을 찾아 하나의 배열에 위치 정보들을 저장한다.
 *   이때, 제거할 수 있는 블록은 현재 위치에서 2x2 크기의 블록들이 전부 동일한 모양인 경우이다.
 *   순회를 마친 뒤, 제거할 수 있는 블록이 존재한다면 전부 제거한다.
 *   블록을 제거한 뒤, 위에 있는 블록을 아래로 떨어뜨려 빈 공간을 채운다.
 *   제거할 수 있는 블록이 존재하지 않을 때까지 위 과정을 반복한다.
 *
 * - 구현 문제로, 문제에서 제시한 조건 그대로 구현하면 되는 문제였다!
 */
