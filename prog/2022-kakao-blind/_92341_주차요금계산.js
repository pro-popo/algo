...
function solution(fees, records) {
    ...
    records.forEach((record) => {
        ...
        switch (type) {
            case 'IN':
                parkingLot.set(car, timeInMinute);
                break;
            case 'OUT':
                const usedTime = timeInMinute - parkingLot.get(car);
    ...
    return Object.entries(totalUsedTime)
        .sort(([carA], [carB]) => +carA - +carB)
        .map(([_, usedTime]) => {
            if (usedTime <= defaultTime) return defaultPrice;
            return (
                defaultPrice +
